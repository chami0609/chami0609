...
def cross_validate_with_improvements(
    ...
    for fold in range(n_folds):
        ...
        epoch_pbar = tqdm(
            ...
            ncols=120,
            leave=False,  # 完了後に消す（次のFoldと重ならない）
            position=0,
            dynamic_ncols=True
        )
        # デバッグ用：時間計測とリソース監視
        ...
        if fold == 0:
            ...
            print(f"  CPU: {psutil.cpu_count()} cores, Memory: {initial_memory:.2f} GB")
            if device == 'cuda' and torch.cuda.is_available():
                print(f"  GPU: {torch.cuda.get_device_name(0)}, "
                    f"Mem: {torch.cuda.memory_allocated(0)/1024**3:.2f}GB")
            print()
        ...
        for epoch in epoch_pbar:
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                # 最初の数バッチの詳細情報を表示
                if fold == 0 and epoch == 0 and batch_idx < 3:
                    tqdm.write(f"  [B{batch_idx}] Load:{(t1-t0)*1000:.0f}ms Fwd:{(t3-t2)*1000:.0f}ms Bwd:{(t4-t3)*1000:.0f}ms")
            ...
            if fold == 0 and epoch == 0:
                tqdm.write(f"  [Timing] Val: {val_time:.1f}s")
            # 学習曲線データの保存
            ...
            f"Loss:{avg_val_loss:.3f} F1:{val_f1:.3f} Best:{best_val_f1:.3f}"
            )
            ...
            if early_stopping.early_stop:
                epoch_pbar.close()
                tqdm.write(f"  ⏹️ Early stop E{epoch+1} (Best: {best_val_f1:.4f} @E{best_epoch})")
                break
        ...
        plot_confusion_matrix(
            ...
            save_path=os.path.join(model_save_dir, f'{model_name}_confusion_matrix_fold_{fold+1}_final.png'),
            normalize=False,
            thresholds=fixed_thresholds
        )
        plot_confusion_matrix(
            ...
            save_path=os.path.join(model_save_dir, f'{model_name}_confusion_matrix_fold_{fold+1}_final_normalized.png'),
            normalize=True,
            thresholds=fixed_thresholds
        )
        ...
        if best_val_preds is not None:
            ...
            plot_confusion_matrix(
                ...
                save_path=os.path.join(model_save_dir, f'{model_name}_confusion_matrix_fold_{fold+1}_best.png'),
                normalize=False,
                thresholds=fixed_thresholds
            )
    ...
    predictions_path = os.path.join(model_save_dir, f'{model_name}_all_predictions.csv')
    ...
    plot_confusion_matrix(
        ...
        save_path=os.path.join(model_save_dir, f'{model_name}_confusion_matrix_average.png'),
        normalize=False,
        thresholds=fixed_thresholds
    )
    plot_confusion_matrix(
        ...
        save_path=os.path.join(model_save_dir, f'{model_name}_confusion_matrix_average_normalized.png'),
        normalize=True,
        thresholds=fixed_thresholds
    )
    ...
    # JSONファイルに保存
    with open(os.path.join(model_save_dir, f'{model_name}_cv_results.json'), 'w') as f:
...
def main():
    ...
    classification_mode = "5class"  # ← ここを "3class" に変更すると3値分類になります
    ...
    #
    model_names = ["CNN", "ViT", "MLP-Mixer", "HybridModel"]  # 使用するモデル名のリスト
    ...
    dataset_entries = [
        ...
        ('07ee702', data_sample_dir / '07ee702.jpg', data_annotation_dir / '07ee702' / '07ee702.geojson'),
    ]
    ...
    config = {
        ...
        # ハイパーパラメータ探索のON/OFF
        'hyperparameter_search': False,  # Falseで探索をスキップ
        ...
        # CNN用の定義を追加
            ...
            'HybridModel': {
                ...
                'learning_rate': 0.0002607965659809582,  # 学習率を調整
                'batch_size': 128,
                'weight_decay': 1.9762189340280066e-05,
                "label_smoothing": 0.1,
                'in_channels': expected_in_channels
            },
        },
