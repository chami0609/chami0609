...
def print_fold_statistics_improved(fold_assignments, landslide_ids, labels):
    ...
    for fold in range(n_folds):
        ...
        print(f"    Fold {fold+1}: {count:6,} ({percentage:5.1f}%) [{deviation:+5.1f}% from target]")
...
def visualize_balanced_spatial_kfold(gdf_with_folds, config):
    ...
    stats_text += '\n'.join([f'Fold {f+1}: {c:,}' for f, c in fold_counts.items()])
...
# *******************************************************************
class CNN(nn.Module):
    def __init__(self, in_channels=3, num_classes=5, hidden_dim=128, depth=4, kernel_size=3, maxchannels=512):
        ...
        for i in range(depth):
            out_channels = min(hidden_dim * (2 ** i), maxchannels)
...
def plot_cv_scores_enhanced(fold_results, save_dir, model_name, class_names=None):
    ...
    fold_colors = [palette[i % len(palette)] for i in range(n_folds)]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'{model_name}_cv_metrics_summary.png'), dpi=300, bbox_inches='tight', facecolor='black')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'{model_name}_cv_metrics_by_fold.png'), dpi=300, bbox_inches='tight', facecolor='black')
    plt.close()
    ...
    if 'f1' in fold_results[0]['metrics']:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'{model_name}_cv_f1_heatmap.png'), dpi=300, bbox_inches='tight', facecolor='black')
        plt.close()
...
def evaluate_and_save_ensemble(model_names, results, config):
    ...
    predictions_path = ensemble_dir / 'Ensemble_all_predictions.csv'
    ...
    for fold_result in fold_results:
        ...
        plot_confusion_matrix(
            ...
            save_path=ensemble_dir / f'Ensemble_confusion_matrix_fold_{fold_idx}.png',
            normalize=False,
        ...
        plot_confusion_matrix(
            ...
            save_path=ensemble_dir / f'Ensemble_confusion_matrix_fold_{fold_idx}_normalized.png',
            normalize=True,
            thresholds=ensemble_thresholds
        )
    ...
    plot_confusion_matrix(
        ...
        save_path=ensemble_dir / 'Ensemble_confusion_matrix_average.png',
        normalize=False,
        thresholds=ensemble_thresholds
    )
    ...
    print("\n✅ Ensemble summary saved to:", ensemble_dir / 'Ensemble_cv_results.json')
